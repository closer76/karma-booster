...
namespace PlurkApi
{
    ...
    public class PlurkFriend
    {
        ...
        public PlurkFriend(string jsonString)
        {
            ...
            try { this.nick_name = ( (JsonString)((JsonObject)jsonObject[uid.ToString()])["nick_name"]).Value; }
            catch { this.nick_name = ""; }
            ...
            try { this.full_name = ((JsonString)((JsonObject)jsonObject[uid.ToString()])["full_name"]).Value; }
...
//            data = web.GetPage("http://www.plurk.com/Users/getFriends?user_id=" + uid.ToString(), null, ref cookie, true);
            ...
            foreach (string item in arrayData)
            {
                ...
                if (jsonString[jsonString.Length - 1] == '}' && jsonString[jsonString.Length-2] != '}')
                {
                    jsonString = jsonString + "}";
                }
                else if (jsonString[jsonString.Length - 1] != '}')
                {
                    jsonString = jsonString + "}";
                    jsonString = jsonString + "}";
                }
